...
You can check the `node_modules/nextra-theme-docs/style.css` file to see the available class names.

To style a component with Nextra's Tailwind CSS, just use Tailwind CSS syntax with `nx-` prefix. If it doesn't work, it means that that class name doesn't exist in the `style.css` file.
