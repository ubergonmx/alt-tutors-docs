...
Follow steps 1-3 if you're starting a new task. If you're done with the task, follow steps 4-7. If you're done with the task and you're ready to merge, follow steps 8-10.
...
- Check the PR if it follows the [pull request format](#pull-request-pr).
- Check the code if it follows the [coding standards](/getting-started/workflow/standards). If it doesn't, you can comment to the PR (or file) to link them to the coding standards page with the hyperlink. Example:

```md
Not following [folder naming convention](https://alt-tutors-docs.vercel.app/getting-started/workflow/standards#files-and-folders) for `signUp`. Please rename to `sign-up` folder.
```

- Check if the code is working as intended. If there are errors/bugs, comment them with the screenshot and the possible solution.
- Check if the code is documented properly.
- Check if the code is tested properly.
...
After merging the PR, delete the branch. This is to avoid confusion when creating a new branch.
