...
<Callout>
  ...
  identifier. For this, we will use [S-I-D](/getting-started/primer/naming#s-i-d) technique.
</Callout>

#### Files and Folders

- **File paths** in `import` must be **case-sensitive**. This is to avoid issues when deploying to Linux servers (which Vercel uses).
...
Use absolute imports for better organization. Structure the folder hierarchy logically.
...
Use code formatting tools like Prettier to maintain consistent formatting.
